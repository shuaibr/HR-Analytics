...
univariate.stats <- function( vname, vlabel=deparse(substitute(vname)), ...) 
{
     ...
     tmp <- vname[vname != 0.0 & is.na(vname)==F]
     nz <- length(tmp)
...
remove_missing <- function( data, cnames, missingval)
{
     dd <- data.frame(data)
     ds <- NULL
     ds <- which(apply(dd[,cnames] == missingval,1,any))
     if (length(ds) > 0) dd <- dd[-ds,]
     cat("\nObservations in working set: ",nrow(dd),"\n")
     dd
}


winsorize <- function( data, cnames, lowertail, uppertail)
{
     dd <- data.frame(data)
     for (i in 1:length(cnames)) {
        x = cnames[[i]]
        tails = quantile(dd[[x]], c(lowertail, uppertail))
        dd[[x]][ dd[[x]]< tails[[1]] ] <- tails[[1]]
        dd[[x]][ dd[[x]] > tails[[2]] ] <- tails[[2]]
     }
     dd
}


quick.summary <- function( vname, vlabel=deparse(substitute(vname)), ...) 
{
     # quick summary of vname 
     n <- length(vname)
     n <- as.integer(n)
     mn <- mean(vname)
     mdn <- median(vname)
     ...
     cat("Min: ",mnm,", ")
     cat("Max: ",mxm,"\n\n")
     unv<- c(n, mn, sdev, mdn, mnm, mxm)
     names(unv) <- c('obs', 'mean', 'stdev', 'median', 'min', 'max')
     quick.summary <- unv
}
